...
export async function POST(request: Request) {
  try {
    ...
    // Run 2 parallel calls — each generates 2 documents (~25s each vs ~60s+ for all 4)
    ...
    return Response.json({ error: "Failed to generate assessment documents" }, { status: 500 });
  }
}
